...
class Model_listings extends MY_Model
{
   ...
   public function setLimit($limit) {
	...
	public function getAllActiveListing($Cat = null) {
	 ...
	 public function getAllActiveListingBySouCat($SouCat = null ,$wilaya = null) {
	...
	public function remove($id)
	{
		if($id) {
			...
			$deleteF = $this->db->delete('listing_like');
...
// then increase by one 
 ...
 public function getListingByTilteAndWilaya($title , $wilaya){
			    ...
			    $this->db->where('contact_info.wilaya' , $wilaya); 
                $this->db->where('listings.status' , 1); 
		              $this->db->where("CONCAT(listings.Title, ' ', sou_category) LIKE '%$title%'");
			    $query = $this->db->get('listings');
                return $query->result_array(); 
 } 
 ...
 public function getListingDistance($title , $lat , $lng ,  $filtr , $wilaya = null){
	
	  
	if( $wilaya != null && $wilaya != 'Autour de moi'){
		
		 $sql = "SELECT * FROM wilaya WHERE name = ?";
			$query = $this->db->query($sql, array($wilaya));
			$r = $query->row_array();
        if($query){
			$idWilaya =$r['id'];
			$latwilaya =$r['lat'];
	...
	$radius = 20;
	$sql = "SELECT listings.* , contact_info.lat ,contact_info.lng, contact_info.wilaya ,contact_info.id as cordId,
	( 6371 * acos( cos( radians('$latwilaya') ) * cos( radians( contact_info.lat) ) * cos( radians( contact_info.lng) - radians('$lngwilaya') )
	+ sin( radians('$latwilaya') ) * sin( radians( contact_info.lat) ) ) ) AS distance FROM listings,contact_info WHERE 
	listings.id = contact_info.listing_id AND listings.status = 1 AND contact_info.wilaya = $idWilaya AND Title LIKE '%$title%'
	HAVING distance < '$radius' ORDER BY distance LIMIT 0 , 40";
	  ...
	  $name = 1 ;
	    ...
	    $sql = "SELECT listings.* , contact_info.lat ,contact_info.lng, contact_info.wilaya ,contact_info.id as cordId,
	    ( 6371 * acos( cos( radians('$lat') ) * cos( radians( contact_info.lat) ) * cos( radians( contact_info.lng) - radians('$lng') )
	     + sin( radians('$lat') ) * sin( radians( contact_info.lat) ) ) ) AS distance FROM listings,contact_info WHERE 
	    listings.id = contact_info.listing_id AND listings.status = 1 AND (Title LIKE '%$title%' OR  category  = '$name') 
    	HAVING distance < '$radius' ORDER BY distance LIMIT 0 , 30";
	}
