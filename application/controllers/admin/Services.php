...
class Services extends Admin_Controller 
{
	...
	{   $data = array();
		if($id) {
			$listing = $this->model_services->getServiceData($id);
  ...
  $photo = ' <div class="thumbnail" style="width:150px">
        <a href="'.base_url('listing_detail/index/'.$value['id']).'" target="_blank">
          <img src="'.base_url('assets/images/services/'.$value['photo']).'" alt="Fjords" >
       </a>
 </div> ';	  
	...
	public function update ($id) {
		...
		if($id) {
	        	...
	        	$update = $this->model_services->update_service($data, $id);
			 	$lis = $this->model_services->getServiceData($id);
   ...
   public function add_listing () { 
	...
	if ($this->form_validation->run() == TRUE) { 
	 ...
	 $listings = $this->model_services->getListingByPlaceId($place_id);  
	...
	if( $listings > 0){	
       ...
       redirect('admin/Listings/add_listing?Category='.$Category.'&sou_category='.$sou_category.'&place_types='.$place_types.'&wilaya='.$wilaya.'&Zip-Code='.$Zip, 'refresh');
	     ...
	     $insert = $this->model_services->create_Listing($data);
	    ...
	    $insert_info = $this->model_services->create_ContactInfo($info);
...
function get_furl($url)
  ...
  public function getinfo ($cat , $wilaya){
	 ...
	 $listing  = $this->model_services->getListingBySouCat($cat, $wilaya);
	  ...
	  foreach($listing as $key => $value) {
             ...
             $coords = $this->model_services->getContactInfoData($value['id']); 
