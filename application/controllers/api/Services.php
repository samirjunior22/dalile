...
class Services extends \Restserver\Libraries\REST_Controller
{
    ...
    if ($this->form_validation->run() == TRUE) { 
		  ...
		  $listingPhoto ="service.jpg";  
              ...
              $find = array("[","]");
              $wilaya_list = str_replace($find,'',$this->input->post('wilaya'));
 
               $wilaya = explode(',', $wilaya_list);
    ...
    public function detail_get(){
		 ...
		 $listings = $this->listings->getListingData($id); 
		  ...
		  $this->data['allPhoto']= $this->listings->getAllImages($listings['id']); 
	...
	public function upload_image_post () {
	     ...
	     $type = $this->input->post('type') ;
	      if( $type == 'place'){
			    $content = 'assets/images/listings/' ; 
			  
		  } else {
			 $content = 'assets/images/services/' ;
            ...
            file_put_contents($content. $filename, $image);
			  
			  $data= array( 
				'photo' => $filename,
				 );
			  if( $type == 'place'){
			  	   $insert = $this->listings->update_Listing($data, $id);
			    } else { 
    		    $insert = $this->model_services->update_Service($data, $id);
		       }
